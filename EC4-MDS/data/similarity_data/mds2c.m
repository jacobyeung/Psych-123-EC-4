function [points,vaf]=mds2c(distance,dimensions,metric)

% MDS multidimensional scaling, version 2 with additive constant (michael.d.lee@dsto.defence.gov.au)
...
options=zeros(18,1);
options(1)=-1; %display
options(2)=1e-2; %x precision
options(3)=1e-4; %residuals precision
...
if dim==1
   p0=[p0;rand*.05];
   %p0=[classicalmds(d,1);0];
else
   p0=[p0;rand*.05 zeros(1,dim-1)];
end;
%p=leastsq('mdsresidualsc',p0,options);
[p sse]=lsqnonlin('mdsresidualsc',p0,zeros(n+1,1),[]);
%resid=mdsresidualsc(p);
